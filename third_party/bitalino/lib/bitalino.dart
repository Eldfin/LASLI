...
final Duration timeout = Duration(seconds: 35);
...
class BITalinoDevice {
  const BITalinoDevice({
    required this.name,
  ...
  factory BITalinoDevice._fromPlatformData(Map<dynamic, dynamic> data) {
    final type = data['type'] == 2 ? CommunicationType.BLE : CommunicationType.BTH;
...
class BITalinoController {
  ...
  static Future<List<BITalinoDevice>> scanDevices(
    ...
    return (devices ?? const [])
        .map((device) => BITalinoDevice._fromPlatformData(device))

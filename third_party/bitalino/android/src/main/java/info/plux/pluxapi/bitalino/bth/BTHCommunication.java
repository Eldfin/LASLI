...
//        mConnectedThread.setPriority(Thread.MAX_PRIORITY);
    ...
    private class ConnectThread extends Thread {
        ...
        public void run() {
            ...
            BluetoothSocket connectedSocket = connectWithFallbacks(mmSocket);
            ...
            connected(connectedSocket, mSocketType);


        }

        private BluetoothSocket connectWithFallbacks(BluetoothSocket secureSocket) {
            BluetoothSocket socket = secureSocket;
            if (tryConnect(socket, "Secure")) {
                return socket;
            }
            closeSocket(socket, "secure socket during connection fallback");

            try {
                socket = mmDevice.createInsecureRfcommSocketToServiceRecord(MY_UUID_SECURE);
                if (tryConnect(socket, "Insecure")) {
                    return socket;
                }
                closeSocket(socket, "insecure socket during connection fallback");
            } catch (IOException e) {
                Log.e(TAG, "Insecure socket create() failed", e);
            }

            socket = createRfcommSocket("createRfcommSocket");
            if (socket != null) {
                if (tryConnect(socket, "RfcommChannel1")) {
                    return socket;
                }
                closeSocket(socket, "RFCOMM channel 1 socket during connection fallback");
            }

            socket = createRfcommSocket("createInsecureRfcommSocket");
            if (socket != null) {
                if (tryConnect(socket, "InsecureRfcommChannel1")) {
                    return socket;
                }
                closeSocket(socket, "insecure RFCOMM channel 1 socket during connection fallback");
            }

            return null;
        }

        private boolean tryConnect(BluetoothSocket socket, String socketType) {
            if (socket == null) {
                return false;
            }

            try {
                Log.d(TAG, "Trying " + socketType + " socket.");
                socket.connect();
                Log.d(TAG, socketType + " socket connected.");
                return true;
            } catch (IOException e) {
                Log.e(TAG, socketType + " socket connect() failed", e);
                return false;
            }
        }

        private BluetoothSocket createRfcommSocket(String methodName) {
            try {
                Method method = mmDevice.getClass().getMethod(methodName, int.class);
                return (BluetoothSocket) method.invoke(mmDevice, 1);
            } catch (Exception e) {
                Log.e(TAG, methodName + "(1) failed", e);
                return null;
            }
        }

        private void closeSocket(BluetoothSocket socket, String description) {
            if (socket == null) {
                return;
            }
            ...
            try {
                ...
                Log.e(TAG, "unable to close " + description, e);
            }
        }
